  ...
  (define N
   ...
   (define (find-extension-in-edge skipped-node skip-offset i)
     (cond [(label-element-equal?
            ...
            (let ([n (add1 i)]) ;(unless (index? n) (error "find-next"))
                    (loop-rest n))]
...
(define (suffix-tree-add! tree label)
  ...
  (define (add-rest-suffixes! label starting-node starting-offset)
    (add-rest-suffixes-loop!
     label
     (let ([i (label-length label)]) ;(unless (index? i) (error "ars"))
           i)
          ...
          (begin
            (let ([new-active-node
                  ...
                  (add-rest-suffixes-loop!
                   label
                   N
                   (let ([num (max i* (add1 j))]) ;(unless (index? num) (error "foo"))
                       num)
                   (let ([num (add1 j)]) ;(unless (index? num) (error "foo"))
                       num)
                   new-active-node)))
