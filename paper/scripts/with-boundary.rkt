...
(define (show-boundary S data-modules)
  ...
  (when (not (null? (cdr (car data+required-by))))
    ...
    (define with-data    (predicate->variations S has-boundary?))
    (define without-data (predicate->variations S no-boundary?))
    (define (stream->mean s)
     (define-values (len sum)
       (for/fold ([len 0] [sum 0])
                 ([v (in-stream s)])
         (values (add1 len) (+ (variation->mean-runtime S v) sum))))
     (when (zero? len)
