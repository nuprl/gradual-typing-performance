...
(module+ main
  ...
  (for ([pn (in-list ARG*)])
    ...
    (define var+mean* (for/list ([v (all-configurations S)])
                        (cons v (configuration->mean-runtime S v))))
    ...
    (with-output-to-file out-file #:exists 'replace
      (lambda ()
        (printf ";; ~a\n;; ~a\n;;   Config.   Runtime (ms)    Overhead (X worse than untyped)\n" name (make-string (string-length name) #\=))
        (displayln "(")
        (for ([v+r (in-list (sort var+mean* > #:key cdr))])
          (printf "(~a   ~a   ~a)\n" (car v+r) (my-round (cdr v+r)) (my-round (/ (cdr v+r) baseline))))
        (displayln ")")))
