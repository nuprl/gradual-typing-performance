     ...
     (λ (blame)
       (let ([predicate-contract-proj
              (λ (v)
                (if (p? v)
                    (λ (neg-party)
                      ;;bg; check for specially-tagged neg-parties.
                      ;;    if found, send a message that we just checked a contract
                      (when (and (list? neg-party) (eq? 'bg-typed-contract (car neg-party)))
                        (printf "[BG:APPLY]\t~a\n" (cadr neg-party)))
                      v)
                    (λ (neg-party)
