...
(: try-add-rectangle (-> Grid Pos Index Index (->* (Pos) (Index) Pos) (U #f Room)))
(define (try-add-rectangle grid pos height width direction)
  ...
  (define-values (success? poss->cells free-cells extension-points)
    (for*/fold : (Values Boolean Poss->Cells (Listof Pos) (Listof Pos))
        ...
        ([x0 : Integer (in-range min-x max-x)]
         [y0 : Integer (in-range min-y max-y)])
      ;#:break (not success?)
      (define x : Index (cast x0 Index))
      (define y : Index (cast y0 Index))
      (define p : Pos (vector x y))
