...
(only-in "array-struct-unsafe-build-array.rkt"
  unsafe-build-array)
(only-in "array-broadcast-array-broadcast.rkt"
  array-broadcast)
(only-in "array-broadcast-array-shape-broadcast.rkt"
  array-shape-broadcast)
(only-in "array-utils-unsafe-vector-remove.rkt"
  unsafe-vector-remove)
(only-in "array-utils-vector-copy-all.rkt"
  vector-copy-all)
(only-in "array-utils-unsafe-vector-insert.rkt"
...
(define (array-append* arrs [k 0])
  ...
  (let-values ([(arrs dks)  (array-broadcast-for-append arrs k)])
    ...
    (cond
      [(> 0 new-dk)  (error 'array-append* "resulting axis is too large (not an Integer)")]
      [else
...
(define (array-broadcast-for-append arrs k)
  (define dss
    (for/list
      ([arr arrs])
      (Array-shape arr)))
  (define dims (apply max (for/list
                            ([ds dss])
                            (vector-length ds))))
  ;(define dims (apply max (map vector-length dss)))
  (cond [(> 0 dims)  (error 'array-broadcast-for-append "can't happen")]
        [(or (k . < . 0) (k . >= . dims))
