...
(require
 benchmark-util
 "graph-types.rkt")
 ;; "../base/t-graph-types.rkt")
(require/typed/check "graph-attach-edge-property.rkt"
...
;; ---------------------------------------------------------------------------------------------------
(: generate-graph (-> Connection* Line->Connection* (Values unweighted-graph (Station Station -> [Setof Line]))))
...
(define mbta%
  (class object% (init-field G stations connection-on bundles)
    ...
    (define/public (station word)
      ...
      (: candidates Station*)
      (define candidates
        (for/list : Station* ([s : Station stations-set] #:when (regexp-match word# s))
          s))
        ...
        (cond
          ...
          [else
           ...
           (for/fold : [Listof [Pairof Station Station*]] 
             ...
             ([n ((unweighted-graph-in-neighbors G) from)])
