...
(define (evolve population cycles rate rounds)
  (define-values (result _)
    (for/fold ([result ;:[Listof Payoff]
                '()]
               [population ;:[Population Automaton]
                population])
              ([_ (in-range cycles)])
      [define payoffs (match-ups ;Automaton)
                        population rounds interact)]
      (values ( cons ;Real [Listof Payoff]}
                (relative-average payoffs rounds) result)
...
(define (payoff-percentages payoffs)
  ...
  (define-values (accumulated _)
    (for/fold ([accumulated ;: [Listof Real]
                (list)]
               [init ;: Real
                0])
               ([y ;: Real
      ...
      (values ( cons ;Real [Listof Real]}
                next-init accumulated) next-init)))
  (reverse accumulated))
