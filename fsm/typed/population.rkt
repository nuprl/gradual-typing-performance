...
(define (match-ups population0 rounds-per-match interact)
  ...
  (define (match-up auto1 auto2)
    (for/fold ([sum1 : Real 0] [sum2 : Real 0] [auto1 : a auto1] [auto2 : a auto2])
...
(define (randomise-over-fitness population fitness speed)
  (for/list ([n : Integer (in-range speed)])
    [define r (random)]
    [define xxx (vector->list population)]
    (car
     (first (filter (lambda ({p+f : [cons a Real]}) (< r (cdr p+f)))
                    ({inst map [cons a Real] a Real}
                     cons
                     xxx fitness))))
    #;
    (for/last ([p : a    (in-vector population)]
               [f : Real (in-list fitness)]
               #:break (< r f))
      p)))
