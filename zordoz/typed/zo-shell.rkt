...
;;         [arg "dive 0"])
;;     (define-values (ctx* hist* pre*) (dive arg ctx hist pre-hist))
;;     (check-equal? ctx*  (result-z (car ctx)))
;;     (check-equal? pre-hist pre*)
;;     (check-equal? hist* '()))
...
;;         [arg "3"])
;;     (let-values ([(ctx* hist*) (dive-list ctx hist arg)])
;;       (begin (check-equal? ctx*  (result-z (cadddr ctx)))
...
;;       (begin (check-equal? (result-z (car ctx*)) z)
...
;;              (check-equal? (result-z (car ctx*)) (branch-else ctx))
